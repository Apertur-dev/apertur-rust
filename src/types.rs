...
pub struct UploadRecord {
    ...
    /// Environment (live or test). Not always present on list responses.
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Stats {
    /// Number of sessions created in the reporting range (current period).
    ///
    /// The API returns this under the `sessionsInRange` key; the legacy
    /// `sessionsThisMonth` key is also accepted for backward compatibility.
    #[serde(rename = "sessionsInRange", alias = "sessionsThisMonth")]
