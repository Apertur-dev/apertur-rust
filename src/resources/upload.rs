...
impl Upload {
    ...
    pub fn image_encrypted(
        ...
        // In `default` encryption mode the server reads MULTIPART form data, takes
        // the bytes of the "file" part, parses them as the JSON envelope
        // {encryptedKey, iv, encryptedData, algorithm}, then decrypts server-side.
        // So we send the serialized envelope as the file bytes (NOT as a JSON body).
        let envelope = serde_json::to_vec(&encrypted)?;

        let file_part = reqwest::blocking::multipart::Part::bytes(envelope)
            .file_name("image.jpg.enc")
            .mime_str("application/octet-stream")
            .map_err(|e| AperturError::Encryption(format!("Invalid MIME type: {}", e)))?;

        let mut form = reqwest::blocking::multipart::Form::new().part("file", file_part);

        // Only send `source` when the caller provides it. The server validates it
        // against the ImageSource enum and rejects unknown values (e.g. "sdk") with
        // a 400; omitting it lets the server default to "camera".
        if let Some(ref source) = options.source {
            form = form.text("source", source.clone());
        }
